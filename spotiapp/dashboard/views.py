...
def recently_played(request):
    ...
    recently_played_user = sp.current_user_recently_played(limit=50, after=None, before=current_timestamp)
    recently_played = recently_played_user['items']
    
    song_info = []
    for index, artist in enumerate(recently_played, start = 1):
        song_info = {
            'index': index,
            'song': artist['song'],
        ...
        song_info.append(song_info)
    
    return render(request, 'recently_played.html', {'artists_info': song_info, 'time_range': time_range})
